...
def train():
    ...
    # --- Bulked Up ~500M Parameter Model ---
    model = JesperLLM(
        vocab_size=65536, 
        pad_id=tokenizer.pad_token_id, 
        max_seq_len=max_seq_len,
        dim=1024,             # Increased from 768
        n_layers=8, 
        n_heads=16,           # Increased from 8
        n_kv_heads=4,         # Increased from 2
        hidden_dim=2048,      # Increased from 1024
        num_experts=8, 
        top_k=2
    ).to(local_rank)
    ...
    # Defaults to FULL_SHARD (1x8). To use HYBRID_SHARD (4x2), you would define a process group 
    # and pass sharding_strategy=ShardingStrategy.HYBRID_SHARD
    model = FSDP(
        model, 
    ...
    if local_rank == 0:
        print(f"Booting 500M Jesper on 65k Vocab via FSDP. Global batch size: {batch_size * world_size * accumulation_steps}")
